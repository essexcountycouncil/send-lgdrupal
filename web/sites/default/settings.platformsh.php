...
// depending on how the Host header is used. However, Platform.sh already
// replaces the Host header with the route that was used to reach Platform.sh,
...
foreach ($platformsh->variables() as $name => $value) {
  ...
  switch ($prefix) {
    ...
    // Example: Variable `d8config:conf_file:prop:subprop` with value `foo`
    // becomes $config['conf_file']['prop']['subprop'] = 'foo';
    // Example: Variable `d8config:conf_file:prop:subprop` with value
    // ['foo' => 'bar'] becomes
...
$solr_server_name = 'sitewide_search_solr';
if ($platformsh->hasRelationship($relationship_name)) {
  $platformsh->registerFormatter('drupal-solr', static function ($solr) {
    ...
    return [
      'core' => substr($solr['path'], 5) ?: 'solr8core',
      'path' => '',
      'host' => $solr['host'],
      'port' => $solr['port'],
    ];
  ...
  // Set the connector configuration to the appropriate value, as defined by the
  // formatter above.
